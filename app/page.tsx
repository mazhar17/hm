...
function HomeContent() {
 ...
 <main><header><div className="breadcrumb">{t("Workspace ")}<span>/</span> <strong>{t(nav.find(n => n.id === view)?.label)}</strong></div><div className="header-right"><select className="language-switch" aria-label={t("Interface language")} value={lang} onChange={e => setLanguage(e.target.value as "en" | "bn")}><option value="en">{t("English")}</option><option value="bn">বাংলা</option></select><span className="saved"><ShieldCheck size={15}/>{demo ? t("Practice · not saved") : t(register.status)}</span><button className="icon" onClick={() => setHelp(true)} aria-label={t("How to use Hifz Mentor")}><CircleHelp size={19}/></button></div></header>
 ...
 {startFor && <Modal title={t("New lesson / test")} onClose={() => setStartFor(null)}><form onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget), from = +String(f.get("from")), to = +String(f.get("to")); if (to < from) {
        toast("The last page must be the same as or after the first page.");
        return;
    } const s = records.students.find(s => s.id === startFor)!; const l = { ...newLesson(s, from, to, f.get("type") as Lesson["type"]), date: String(f.get("date")) }; change({ ...records, lessons: [...records.lessons, l] }); openLesson(l); setStartFor(null); }}><div className="student-label"><span className="avatar">{initials(records.students.find(s => s.id === startFor)?.name || "")}</span><strong>{records.students.find(s => s.id === startFor)?.name}</strong></div><div className="form-grid"><Field label={t("Date")}><input name="date" type="date" required defaultValue={today()}/></Field><Field label={t("Lesson type")}><select name="type" defaultValue="Sabqi"><option value="Sabaq">{t("Sabaq")}</option><option value="Sabqi">{t("Sabqi")}</option><option value="Manzil">{t("Manzil")}</option><option value="Test">{t("Test")}</option></select></Field></div><div className="form-grid"><Field label={t("From page")}><input name="from" type="number" min={1} max={604} required defaultValue={1}/></Field><Field label={t("To page")}><input name="to" type="number" min={1} max={604} required defaultValue={1}/></Field></div><details className="surah-shortcut"><summary>{t("Choose pages by surah")}</summary><Field label={t("Surah")}><select defaultValue="" onChange={e => { const f = e.target.form!; const p = meta?.surahs.find(s => s[0] === +e.target.value)?.[6]; if (p) {
        (f.elements.namedItem("from") as HTMLInputElement).value = String(p);
        (f.elements.namedItem("to") as HTMLInputElement).value = String(p);
    } }}><option value="">{t("Choose a surah\u2026")}</option>{meta?.surahs.map(s => <option key={s[0]} value={s[0]}>{s[0]}. {s[2]} · {s[1]}</option>)}</select></Field></details><div className="modal-actions"><button type="button" onClick={() => setStartFor(null)}>{t("Cancel")}</button><button type="submit" className="primary" disabled={!writable}>{t("Start ")}<ArrowUpRight size={17}/></button></div></form></Modal>}
