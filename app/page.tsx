"use client";
import { mistakeMessage } from "../lib/mistake-message";
import { navigateLesson } from "../lib/lesson-navigation";
...
function lessonTime(value: string | undefined, lang: string) {
    if (!value) return "";
    const date = new Date(value);
    return Number.isNaN(date.getTime()) ? "" : date.toLocaleTimeString(lang === "bn" ? "bn-BD" : "en-GB", { hour: "2-digit", minute: "2-digit" });
}
...
function HomeContent() {
    ...
    function goto(p: number) { if (!current || p === page)
        return; const next = navigateLesson(current, p, writable); if (next.lesson !== current) updateLesson(next.lesson); setPage(next.page); setImageError(false); setImageLoaded(false); }
 ...
 {view === "history" && <><div className="eyebrow">{t("A RECORD OF PROGRESS")}</div><div className="page-title"><div><h1>{t("Lesson history")}</h1><p>{t("Keep the lesson. See the improvement.")}</p></div><select aria-label={t("Filter history by student")} value={studentFilter} onChange={e => setStudentFilter(e.target.value)}><option value="all">{t("All students")}</option>{records.students.map(s => <option value={s.id} key={s.id}>{s.name}</option>)}</select></div><div className="history-list">{[...records.lessons].reverse().filter(l => studentFilter === "all" || l.studentId === studentFilter).map(l => <article className="history-card" key={l.id}><div className="history-date"><BookOpen size={22}/><span>{l.date}</span>{lessonTime(l.completedAt || l.startedAt, lang) && <time className="history-time" dateTime={l.completedAt || l.startedAt} title={t(l.completedAt ? "Completed" : "Started")}>{lessonTime(l.completedAt || l.startedAt, lang)}</time>}</div><div className="history-main"><div className="title-line"><h2>{l.studentName}</h2><span className="pill neutral">{t(l.type)}</span></div><p>{t("Pages ")}{l.from}–{l.to} · {l.marks.length}{t(" corrections \u00B7 ")}{l.status === "draft" ? t("In progress") : t(l.outcome)}</p>{l.assignment && <p className="assignment-preview"><strong>{t("Next:")}</strong> {l.assignment}</p>}<div className="actions compact"><button onClick={() => openLesson(l)}>{l.status === "draft" ? t("Resume lesson") : t("Review corrections")}<ArrowUpRight size={15}/></button>{l.status === "complete" && <button onClick={() => setShareId(l.id)}><Share2 size={15}/>{t("Share record")}</button>}</div></div><span className={`pill ${l.status === "draft" ? "neutral" : "success"}`}>{l.status === "draft" ? t("Draft") : t("Completed")}</span></article>)}{!records.lessons.some(l => studentFilter === "all" || l.studentId === studentFilter) && <div className="welcome"><ClipboardList size={34}/><h2>{t("Every lesson has a place.")}</h2><p>{t("Completed lessons and saved drafts will appear here.")}</p><button className="primary" onClick={() => { setActiveId(null); setView("lesson"); }}>{t("Take a lesson")}</button></div>}</div></>}
 ...
 {view === "settings" && <><div className="eyebrow">{t("YOUR WORKSPACE")}</div><div className="page-title"><div><h1>{t("Settings & records")}</h1><p>{t("A few details that make this teaching desk yours.")}</p></div></div><div className="settings-grid"><div className="panel"><h2>{t("Teaching profile")}</h2><Field label={t("Ustad’s name")}><input placeholder={t("Your name")} maxLength={100} value={records.teacher} disabled={!writable} onChange={e => change({ ...records, teacher: e.target.value })}/></Field><p className="small-muted">{t("Appears on the correction records you share.")}</p></div><div className="panel"><h2>{t("Your records, in your hands")}</h2><p>{t("Download a backup of students, lessons, and every mark. Importing adds records with new IDs and keeps existing records unchanged.")}</p><div className="actions"><button disabled={!writable} onClick={backup}><Download size={17}/>{t("Export backup")}</button><button disabled={!writable} onClick={() => fileInput.current?.click()}>{t("Import backup")}</button></div><input ref={fileInput} type="file" accept="application/json,.json" hidden onChange={async (e) => { const f = e.target.files?.[0]; if (!f)
    ...
    } e.target.value = ""; }}/></div><div className="panel"><ShieldCheck className="teal"/><h2>{t("Private by default")}</h2><p>{t("Student records are saved in your private workspace. A WhatsApp message, exported image, or PDF contains only the selected student\u2019s lesson. Sending remains your choice.")}</p><h4>{t("When the connection drops")}</h4><p>{t("Keep the app open. Changes stay on screen until you retry saving. Download a backup before closing if saving is unavailable.")}</p></div><div className="panel"><h2>{t("About Hifz Mentor")}</h2><p>{t("Designed for the Ustad: listen, mark, and guide.")}</p><p className="small-muted">{t("Madinah Mushaf (Hafs), 604 pages. Page images and Quran metadata supplied with Hifz Companion. The original Quran page files remain unchanged; corrections are stored separately.")}</p><h4>{t("Sources & credits")}</h4><p className="small-muted">{t("Mushaf images: King Fahd Quran Printing Complex, supplied through Hifz Companion. Page metadata: quran-meta (Tanzil-derived). Arabic text: Quran.com. Arabic font: Amiri Quran.")}</p><p className="small-muted"><strong>{t("Word-by-word meanings")}</strong>: {t("Word-by-word meanings are loaded from the Quran.com API v4 (Quran Foundation): English word-by-word translation (QUL resource 92) and Bengali word-by-word translation (QUL resource 94). The Quranic Universal Library credits QuranWBW.com for its word-by-word translations. Glosses are shown only when their Arabic words match the displayed ayah.")}</p><p className="source-links"><a href="https://quran.com/" target="_blank" rel="noreferrer">Quran.com</a> · <a href="https://api-docs.quran.foundation/" target="_blank" rel="noreferrer">Quran Foundation API</a> · <a href="https://qul.tarteel.ai/resources/translation/92" target="_blank" rel="noreferrer">QUL 92 (English)</a> · <a href="https://qul.tarteel.ai/resources/translation/94" target="_blank" rel="noreferrer">QUL 94 (বাংলা)</a> · <a href="https://quranwbw.com/" target="_blank" rel="noreferrer">QuranWBW.com</a></p><button className="text-button" onClick={() => setHelp(true)}>{t("Read the quick guide ")}<ArrowUpRight size={15}/></button></div></div></>}
 ...
 {finish && current && <Modal title={t("Finish lesson")} onClose={() => setFinish(false)}><div className="student-label"><strong>{current.studentName}</strong></div><p>{current.date} · {current.marks.length} {t("corrections")}</p><div className="modal-actions"><button onClick={() => setFinish(false)}>{t("Keep listening")}</button><button className="primary" disabled={!writable} onClick={async () => { if (!writable) return; const completed: Lesson = { ...current, status: "complete", completedAt: new Date().toISOString() }; updateLesson(completed); setFinish(false); setShareId(current.id); try { await navigator.clipboard.writeText(mistakeMessage(completed, records.teacher, lang)); toast("Lesson saved. Message copied."); } catch { toast("Lesson saved. Use Copy message below."); } }}><Copy size={18}/>{t("Save & copy text")}</button></div></Modal>}
 {shared && <Modal title={t("Share the lesson record")} wide onClose={() => setShareId(null)}><div className="share-heading"><CheckCircle2 size={28}/><div><strong>{shared.studentName}</strong><p>{shared.date} · {shared.marks.length} {t("corrections")}</p></div></div><div className="share-options"><button onClick={() => { const c = records.students.find(s => s.id === shared.studentId)?.contact.replace(/\D/g, "") || ""; window.open(`https://wa.me/${c}?text=${encodeURIComponent(mistakeMessage(shared, records.teacher, lang))}`, "_blank", "noopener,noreferrer"); }}><MessageCircle size={18}/>{t("Open WhatsApp")}</button><button className="primary" onClick={async () => { try {
        await navigator.clipboard.writeText(mistakeMessage(shared, records.teacher, lang));
        toast("Lesson message copied.");
    }
    catch {
        download(mistakeMessage(shared, records.teacher, lang), "lesson-record.txt", "text/plain");
        toast("Message downloaded as text.");
    } }}><Copy size={17}/>{t("Copy message")}</button><details><summary>{t("Other export options")}</summary><div className="share-options"><button onClick={() => { try {
        ...
        if (navigator.share)
            await navigator.share({ title: "Hifz Mentor lesson record", text: mistakeMessage(shared, records.teacher, lang) });
        else {
            download(mistakeMessage(shared, records.teacher, lang), "lesson-record.txt", "text/plain");
