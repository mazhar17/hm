...
function HomeContent() {
 ...
 {finish && current && <Modal title={t("Finish lesson")} onClose={() => setFinish(false)}><div className="student-label"><strong>{current.studentName}</strong></div><p>{current.date} · {current.marks.length} {t("corrections")}</p><div className="modal-actions"><button onClick={() => setFinish(false)}>{t("Keep listening")}</button><button className="primary" disabled={!writable} onClick={() => { if (!writable) return; const completed: Lesson = { ...current, status: "complete" }; updateLesson(completed); setFinish(false); setShareId(current.id); const contact = records.students.find(s => s.id === current.studentId)?.contact.replace(/\D/g, "") || ""; window.open(`https://wa.me/${contact}?text=${encodeURIComponent(mistakeMessage(completed, records.teacher, lang))}`, "_blank", "noopener,noreferrer"); }}><MessageCircle size={18}/>{t("Save & open WhatsApp")}</button></div></Modal>}
 {shared && <Modal title={t("Share the lesson record")} wide onClose={() => setShareId(null)}><div className="share-heading"><CheckCircle2 size={28}/><div><strong>{shared.studentName}</strong><p>{shared.date} · {shared.marks.length} {t("corrections")}</p></div></div><div className="share-options"><button className="primary" onClick={() => { const c = records.students.find(s => s.id === shared.studentId)?.contact.replace(/\D/g, "") || ""; window.open(`https://wa.me/${c}?text=${encodeURIComponent(mistakeMessage(shared, records.teacher, lang))}`, "_blank", "noopener,noreferrer"); }}><MessageCircle size={18}/>{t("Open WhatsApp")}</button><button onClick={async () => { try {
