...
export const markSchema = z.object({id, page:z.number().int().min(1).max(604), x:z.number().min(0).max(1), y:z.number().min(0).max(1), category:z.enum(["prompt","word","pronunciation","tajwid","stop","similar"]), location:z.enum(["point","reference"]).optional(), reference:short, note:z.string().max(1000), selfCorrected:z.boolean(), resolved:z.boolean()});
...
export function lessonMessage(l:Lesson,teacher="") {return [`HIFZ MENTOR · LESSON RECORD`,`${l.studentName} · ${l.date}`,`${l.type} · Mushaf pages ${l.from}${l.to!==l.from?`–${l.to}`:""}`,`Outcome: ${l.outcome}`,"",...l.marks.map((m,i)=>`${i+1}. Page ${m.page}${m.reference?` · ${m.reference}`:""}: ${categories.find(c=>c.id===m.category)?.label}${m.selfCorrected?" (self-corrected)":""}${m.resolved?" · Correct on recheck":""}${m.note?` — ${m.note}`:""}`),l.marks.length?"":"No mistakes marked.",l.notes?`Feedback: ${l.notes}`:"",l.assignment?`Next lesson: ${l.assignment}`:"",teacher?`Ustad: ${teacher}`:""].filter((s,i,a)=>s||a[i-1]).join("\n");}
