...
export function lessonMessage(l:Lesson,teacher="",lang:Language="en") {
 const t=(s:string|undefined)=>translate(s,lang);
 return [t("HIFZ MENTOR · LESSON RECORD"),`${l.studentName} · ${l.date}`,`${t(l.type)} · ${t("Mushaf pages")} ${l.from}${l.to!==l.from?`–${l.to}`:""}`,`${t("Outcome")}: ${t(l.outcome)}`,"",
 ...l.marks.map((m,i)=>`${i+1}. ${t("Page")} ${m.page}${m.reference?` · ${m.reference}`:""}: ${t(categories.find(c=>c.id===m.category)?.label)}${m.selectedText?` · “${m.selectedText}”`:""}${m.selfCorrected?` (${t("self-corrected")})`:""}${m.resolved?` · ${t("Correct on recheck")}`:""}${m.note?` — ${m.note}`:""}`),
 l.marks.length?"":t("No mistakes marked."),l.notes?`${t("Feedback")}: ${l.notes}`:"",l.assignment?`${t("Next lesson")}: ${l.assignment}`:"",teacher?`${t("Ustad")}: ${teacher}`:""].filter((s,i,a)=>s||a[i-1]).join("\n");
}
